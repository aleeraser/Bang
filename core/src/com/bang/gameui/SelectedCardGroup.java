...
public class SelectedCardGroup extends Group {
    ...
    public void showCard(Card card) {
        cardImage = CardsUtils.createCardImageGroup(card.getName(), card.getValue(), card.getSuit(),
                height * BOARD_RATIO);
    ...
    public void showCharacterCard(Character character, IPlayer player, int playerNum) {
        ...
        try {
            ...
            try {
                sceneManager.getPlayer().alertPlayerMissing(sceneManager.getPlayer().getPlayers().indexOf(player));
                return;
